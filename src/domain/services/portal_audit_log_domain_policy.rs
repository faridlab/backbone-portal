//! Domain policy for PortalAuditLog aggregate
//!
...
/// Domain policy for PortalAuditLog — permits all operations (no business invariants).
///
...
/// enforcement, implement `backbone_core::ServiceLifecycle<PortalAuditLog>` on a custom
/// service. `PermitAllPolicy` is the safe default until then.
pub type PortalAuditLogDomainPolicy = PermitAllPolicy<PortalAuditLog>;
