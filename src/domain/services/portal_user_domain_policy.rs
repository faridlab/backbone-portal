//! Domain policy for PortalUser aggregate
//!
...
/// Domain policy for PortalUser — permits all operations (no business invariants).
///
...
/// enforcement, implement `backbone_core::ServiceLifecycle<PortalUser>` on a custom
/// service. `PermitAllPolicy` is the safe default until then.
pub type PortalUserDomainPolicy = PermitAllPolicy<PortalUser>;
